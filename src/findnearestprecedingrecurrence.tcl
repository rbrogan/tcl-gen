proc FindNearestPrecedingRecurrence {TargetDatetime RecurrenceStartDatetime RecurrenceIntervalLength} {
     ...
     if {![IsDatetimeQuantity $RecurrenceIntervalLength]} {
          error [format $::ErrorMessage(VARIABLE_CONTENTS_INVALID) RecurrenceIntervalLength $RecurrenceIntervalLength] $::errorInfo $::ErrorCode(VARIABLE_CONTENTS_INVALID)
     }
     ...
     if {$TargetSeconds <= $StartSeconds} {
          for {set TestSeconds $StartSeconds} {$TestSeconds > $TargetSeconds} {Decr TestSeconds $IntervalLengthSeconds} {
          }
          ...
          Decr TestSeconds $IntervalLengthSeconds
          return [eval "clock format $TestSeconds -format $GenNS::DatetimeFormat"]
     }
}